...
const EntityService = {
  ...
  init() {
    this.ready = true;
    Logger.log("EntityService READY v" + this.version);
    return true;
  },
  ...
  getMeta(entity) {
    return EntityRegistry.get(this.resolve(entity));
  },

  getRepository(entity) {
    const name = this.resolve(entity);
    const repo = RepositoryFactory.get(name);
    if (!repo) {
  ...
  create(entity, data = {}) {
    ...
    if (meta.idField && !data[meta.idField]) {
      data[meta.idField] = IdService.generate(entity);
    }

    return this.getRepository(entity).create(data);
  },
  ...
  update(entity, id, data) {
    ...
    const before = this.findById(entity, id);
    if (!before) {
      throw new Error(entity + " not found: " + id);
    }

    return this.getRepository(entity).update(id, data);
  },

  findById(entity, id) {
    return this.getRepository(entity).findById(id);
  },
  ...
  count(entity, criteria = {}) {
    return this.findWhere(entity, criteria).length;
  },
  ...
  bulkCreate(entity, list = []) {
    return list.map(item => this.create(entity, item));
  },
  ...
  transaction(callback) {
    if (typeof TransactionManager !== "undefined" && TransactionManager.run) {
  ...
  health() {
    return HealthContract.create(
      ...
      this.ready ? "OK" : "WARNING",
      {
        ...
        architecture: "Repository Driven Lifecycle",
        features: [
          "CRUD",
          "SEARCH",
          "BULK",
...
globalThis.EntityService = EntityService;